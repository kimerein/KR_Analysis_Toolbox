function FR_across_channelsFig(expt,spikes,unitTag,fileInd,b,saveTag)
%
% INPUT
...
% Created: 11/3/11 - KR
disp('Making FR across channels fig');
...
for m=1:size(cspikes,1)
    ...
    for n=1:size(cspikes,2)
        [h.psth.lines(m,n) temp h.psth.n(m,:,n) centers] = psth(cspikes(m,n),50,h.psth.ax(m),1,duration);
        setTitle(gca,strcat('Ch',num2str(eventChs(m))),7);
    end
end
h.psth.ax=h.psth.ax';
...
defaultAxes(h.psth.ax,0.2,0.08);
ylabel('spikes/s');
...
h.mat(1).params.matpos=[0 0 0.6 1]; % [left top width height]
