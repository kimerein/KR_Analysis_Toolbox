function plotLEDInhVsLEDExcSpecgrams_ForCortex(datadir,dLGNdir,outputDir)
...
if iscell(datadir)
    ...
    for i=1:length(datadir)
        ...
        b=load([lgn '\' 'diffWithLED']);
        currLGN=b.diffWithLED;
...
%     medF1=median(F1s(~isnan(F1s)));
    for k=1:length(noTheta_trialAv_noLED(j).allS.S)
        ...
        lowF1_noTheta_trialAv_noLED(j).S{k}=reshape(nanmean(currs(:,:,currLGN<=0),3),size(currs,1),size(currs,2));
        highF1_noTheta_trialAv_noLED(j).S{k}=reshape(nanmean(currs(:,:,currLGN>0),3),size(currs,1),size(currs,2));
        lowF1(j).S{k}=currs(:,:,currLGN<=0);
        highF1(j).S{k}=currs(:,:,currLGN>0);
    end
end
