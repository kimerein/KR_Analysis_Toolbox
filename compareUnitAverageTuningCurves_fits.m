function [saveStep1,saveStep2,FRsAcrossStims1,FRsAcrossStims2]=compareUnitAverageTuningCurves_fits(allSpikes,unitByUnitTrials,unitByUnitStimcond,unitByUnitLED,useTheseStimcond,useOnlySigPrefUnits,ledCond1,ledCond2,stimResponseWindow,stimResponseLED,saveStep1,saveStep2)
...
if alignToTuning==1
    ...
    for i=1:size(FRsAcrossStims1,1)
        ...
        ft=fittype('@(a,d,b,c,x,y) a*exp(-((x-b)^2)/(2*c^2)) + d*exp(-((x-(b+90))^2)/(2*c^2))');
        ft=fittype(dblgauss);
        fitout=fit(x,y,ft,'StartPoint', [1, 1, 90, 45]);
