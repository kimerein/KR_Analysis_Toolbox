function [ONtrials,OFFtrials,useAssigns,wwidths,cellPhases,isOFF,psths,unitAmps]=separateONandOFFcellsGUI(spikes,useAssigns,ONtrials,OFFtrials)
...
if isempty(ONtrials)
    ...
    % Get unit psths
    [x,psths]=getTrialByTrialUnitPSTH(spikes,useAssigns,trialDuration,bin);
...
%         bestFreq=1.5;
        viewing=downSampAv(nanmean(currcell,1),25);
        ma=max(viewing);
        mi=min(viewing);
        plot(viewing);
        hold on;
        stimviewing=downSampAv(stim_y,25);
        plot(((ma-mi)/2)*(stimviewing-min(stimviewing))+mi,'Color','g');
        hold off;
        isON=questdlg('Is unit ON responsive?');
        if strcmp(isON,'No')
            isOFF(i)=1;
        elseif strcmp(isON,'Yes')
            isOFF(i)=0;
        elseif strcmp(isON,'Cancel')
            return
        end
...
function [x,psths_t]=getTrialByTrialUnitPSTH(spikes,allAssigns,trialDuration,bin)
