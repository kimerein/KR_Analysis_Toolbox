function [fractionpsth,allspikespsth,burstpsth,unitISIplots]=makeSynchPSTH(spikes,psth)

trialDuration=14.5;
doSynchAcrossTrials=0;
...
for i=1:length(psth.psths)
    ...
    if doSynchAcrossTrials==0
        [firstSpikes,followSpikes,preISI,postISI]=sub_findBursts(subspikes.spiketimes,trialDuration);
    else
        [firstSpikes,followSpikes,preISI,postISI]=sub_findBursts(sort(subspikes.spiketimes),trialDuration);
    end
...
function [firstSpikesInBurst,subsequentSpikesInBurst,preISI,postISI]=sub_findBursts(spiketimes,trialDuration)
