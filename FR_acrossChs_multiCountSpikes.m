function FR_acrossChs_multiCountSpikes(expt,spikes,unitTag,fileInd,b,saveTag)
%
% INPUT
...
% Created: 11/3/11 - KR
disp('Making FR across channels fig - count every spike observed on a channel');

RigDef=RigDefs();
disp('Code assumes probe configuration is 1x16 in FR_acrossChs_multiCountSpikes.');
corder=RigDef.ChannelOrder{2};
...
if isempty(saveTag)
    saveTag = [unitTag '_FRacrossChannels'];
else
...
if isstruct(spikes.info.detect) 
    if isfield(spikes.info.detect,'dur')
...
for i=eventChs
    for j=1:length(cond.values)
        %firstTemp=filtspikes(spikes,0,'event_channel',i);
        tSpikes=makeTempField(spikes,'event_channel',i);
        firstTemp=filtspikes(tSpikes,0,'temp',1);
        if strcmp(cond.type,'led')
            tempSpikes=makeTempField(firstTemp,'led',cond.values{j});
            cspikes(q,j)=filtspikes(tempSpikes,0,'temp',1);
        end
    end
    q=q+1;
end

% Resort cspikes and eventChs to match arrangement on probe
useful_corder=corder(1:16)-min(corder(1:16))+1;
eventChs=eventChs(useful_corder);
cspikes=cspikes(useful_corder,:);
...
% Set axes properties
% To set all the same ymax, use this code
...
% defaultAxes(h.psth.ax,0.15,0.05);
% ylabel('spikes/s');

% To use different ymax for each channel, use this code
currAxYmax=findobj(h.psth.ax,'Type','line');
currAxYmaxVec=[];
for i=1:length(currAxYmax)
    temp=get(currAxYmax(i),'YData');
    currAxYmaxVec=[currAxYmaxVec; max(temp)+max(temp)*0.1];
end
for i=1:1
    addStimulusBar(h.psth.ax(i),[w.stim currAxYmaxVec(i)]);
    addStimulusBar(h.psth.ax(i),[w.ledOn currAxYmaxVec(i)*0.97],'','red');
end
removeXAxesLabels(h.psth.ax(1:length(h.psth.ax)-1));
defaultAxes(h.psth.ax,0.15,0.05);
ylabel('spikes/s');
...
h.mat(1).params.matpos=[0.02 0.02 0.94 0.97]; % [left top width height]
...
monitorPosition=[20 0 500 1100];

PaperPosition=[leftMargin bottomMargin (11-leftMargin-rightMargin) (8.5-topMargin-bottomMargin)];
set(hFig,'PaperOrientation','portrait','PaperPosition',PaperPosition,'Color',[0.94 0.94 0.94],'Position',monitorPosition);
