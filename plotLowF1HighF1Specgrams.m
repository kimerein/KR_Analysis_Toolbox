function plotLowF1HighF1Specgrams(datadir,outputDir)
...
for j=1:length(noTheta_trialAv_noLED)
    ...
    for k=1:length(noTheta_trialAv_noLED(j).allS.S)
        ...
        F1s=reshape(nanmean(nanmean(currs(currt>=stimWindow(1) & currt<=stimWindow(2),currf>=F1range(1) & currf<=F1range(2),:),2),1),1,size(currs,3));
%         medF1=median(F1s);
        allFs=reshape(nanmean(nanmean(currs(currt>=stimWindow(1) & currt<=stimWindow(2),currf>=0 & currf<=50,:),2),1),1,size(currs,3));
        F1s=F1s./allFs;
