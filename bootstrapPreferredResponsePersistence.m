function [saveStep1,saveStep2]=bootstrapPreferredResponsePersistence(allSpikes,unitByUnitTrials,unitByUnitStimcond,unitByUnitLED,useTheseStimcond,useOnlySigForStimUnits,PSTHbin,ledOnCond,stimResponseWindow,unitsFractionForBootstrap,trialDuration,saveStep1,saveStep2)
...
precedingWindow=[3.28 3.3]; % s before thalamic silencing
...
for i=1:length(useTheseAssigns)
    if i==1
        ...
        unitByUnitConsensus=currTrialCon(currStimCon==prefStimcond(i) & ismember(currLEDCon,ledOnCond));
        if isempty(unitByUnitConsensus)
            disp('ack');
        end
        ...
        unitByUnitConsensus=currTrialCon(currStimCon==nonprefStimcond(i) & ismember(currLEDCon,ledOnCond));
        if isempty(unitByUnitConsensus)
            disp('ack');
        end
        [~,~,~,xpoints1,ypoints1]=psth_wStd(filtspikes(useSpikes,0,'led',ledOnCond),bin,0,trialDuration,length(unitByUnitConsensus),unitByUnitConsensus);
