function KR_orientation(expt,unitTag,fileInd,b,saveTag,newspikes)
...
global unit_specific_FRs unit_FRs_count

if isempty(unit_FRs_count)
    unit_FRs_count=0;
end

disp('Making KR orientation fig');
varTypeOfInterest='Orientation';
...
%     expt.analysis.other.w.ledOn=str2num(answer{5});
    expt.analysis.other.w.spont=[0 0.5];
    expt.analysis.other.w.stim=[5 8];
    expt.analysis.other.w.on=[5 5.5];
    expt.analysis.other.w.off=[7.5 8];
    expt.analysis.other.w.ledOn=[0.5 4.5];
...
if isempty(cond.color)
    cond.color = {[0.1 0.1 0.1],[1 0.25 0.25],[0 0 1],[1 0.5 0],[1 0 1],[0.3 0.3 0.3],[0.7 0.7 0.7],[0 0.5 1],[0.1 0.8 0.1]};
end
gray = [0.6 0.6 0.6];
green = [0 1 0];
...
if ~isempty(spikes.spiketimes)
    ...
    if isempty(stim.values) | isnan(stim.values)
...
%     disp(stim.code);
        ...
        for n = 1:size(cspikes,2)   % n is number of conditions
            switch label
                ...
                case {'multi-unit','FS multi-unit','axon multi-unit'}
                    h.r.l(m,n) = raster(cspikes(m,n),h.r.ax(m),1,0,duration);
                otherwise
                    % Make last arg = 1 to plot bursts
                    % KR changed
                    h.r.l(m,n) = raster(cspikes(m,n),h.r.ax(m),1,1,duration);
    ...
    for m = 1:size(cspikes,1)       % m is number of stimulus values
        h.psth.ax(m) = axes;
        for n = 1:size(cspikes,2)   % n is number of conditions
            [h.psth.l(m,n) temp h.psth.n(m,:,n) centers] = psth(cspikes(m,n),50,h.psth.ax(m),0,duration);
        end
    end
    h.psth.ax = h.psth.ax';
...
%             disp(unitTag);
%             disp(temp);
            unit_FRs_count=unit_FRs_count+1;
            unit_specific_FRs{unit_FRs_count}=temp;
        end
...
%                         addStimulusBar(h.allp.ax,[0.75 1.25 ymax*0.97],'',[1 0.25 0.25]);
    ...
    h.mat(2).params.matpos = [0.51 0.68 0.23 0.28];
    h.mat(2).params.figmargin = [0 0 0 0];
    h.mat(2).params.matmargin = [0 0 0 0];
    h.mat(2).params.cellmargin = [0.02 0.02 0.02 0.02];
    h.mat(2).ncol = 3;
    h.mat(2).nrow = 2;
    h.mat(2).h(1) = h.pol.stim.ax;
    h.mat(2).h(2) = h.pol.on.ax;
    h.mat(2).h(3) = h.pol.ledOn.ax;
    h.mat(2).h(4) = h.npol.ax;
    h.mat(2).h(5) = h.pol.off.ax;
    
    %     h.mat(2).h(4) = h.pol.off.ax;
    
    h.mat(3).params.matpos = [0.75 0.68 0.25 0.31];
