function alignMUA_UPs_plotUPfamily(expt,mua_xpoints,mua_ypoints,UPstates,useSpontUPs,fileInds,powerRatio)
...
runningUPav=zeros(length(useLED)*20,length(mua_xpoints));
c=1;
UPstate_mua=[];
UPstate_pr=[];
        ...
        if useStimcond(i)==9 & ismember(useLED(i),[1 3 5 7])
            ...
            for j=1:size(ups,1)
                ...
                elseif ups(j,1)<1.3 & ups(j,2)>1.3 & ismember(useLED(i),[3 5])
                    ...
                    if length(curr_runningUP)>size(runningUPav,2)
                        curr_runningUP=curr_runningUP(1:size(runningUPav,2));
                    elseif length(curr_runningUP)<size(runningUPav,2)
                        curr_runningUP=[curr_runningUP zeros(1,size(runningUPav,2)-length(curr_runningUP))];
                    end
                    runningUPav(c,:)=curr_runningUP;
                    c=c+1;
...
figure(); 
for i=2:20:c-1
    plot(mua_xpoints,smooth(runningUPav(i,:),10));
    hold all;
end