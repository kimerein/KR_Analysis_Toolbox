function [lowF1Trials,highF1Trials]=findTrialsWithHighOrLowNtsr1(noTheta_trialAv_noLED,psth,uses,uses_tri,noThetaTrials,isNoTheta,stimWindow)
...
for j=1:length(noTheta_trialAv_noLED)
    ...
    for k=1:length(noTheta_trialAv_noLED(j).allS.S)
        ...
        allFs(k,:)=reshape(nanmean(nanmean(currs(currt>=stimWindow(1) & currt<=stimWindow(2),currf>=0 & currf<=330,:),2),1),1,size(currs,3));
    end
