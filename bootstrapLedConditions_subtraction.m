function [saveStep2]=bootstrapLedConditions_subtraction(allSpikes,useTheseAssigns,unitByUnitTrials,unitByUnitStimcond,unitByUnitLED,type1Stimcond,type2Stimcond,ledCond1,ledCond2,PSTHbin,stimResponseWindow,unitsFractionForBootstrap,trialDuration,saveStep2)

nBootstrapTrials=1000;
saveSteps=1;
ledOnset=1.3; % in seconds from the start of the trial
normalizePrecedingWindow=0;
precedingWindow=[1.28 1.3]; % s before thalamic silencing
zeroWindow=[1.32 1.34]; % For normalization, when traces should be at zero
...
figure(); 
hax=axes();
running_pref_ypoints=1.4117-running_pref_ypoints;
...
hold on; 
running_nonpref_ypoints=2.537-running_nonpref_ypoints;
...
for i=1:length(useTheseAssigns)
    if i==1
        useSpikes=filtspikes(spikes,0,'assigns',useTheseAssigns(i),'stimcond',type1Stimcond{1});
        currTrialCon=unitByUnitTrials{i};
        currStimCon=unitByUnitStimcond{i};
        currLEDCon=unitByUnitLED{i};
        unitByUnitConsensus=currTrialCon(ismember(currStimCon,type1Stimcond{1}) & ismember(currLEDCon,ledCond1{1}));
        if isempty(unitByUnitConsensus)
            disp('ack');
        end
        [~,~,~,xpoints1,ypoints1]=psth_wStd(filtspikes(useSpikes,0,'led',ledCond1{1}),bin,0,trialDuration,length(unitByUnitConsensus),unitByUnitConsensus);
        %%%%%%%%%%%
        useSpikes=filtspikes(spikes,0,'assigns',useTheseAssigns(i),'stimcond',type1Stimcond{2});
        unitByUnitConsensus=currTrialCon(ismember(currStimCon,type1Stimcond{2}) & ismember(currLEDCon,ledCond1{2}));
        if isempty(unitByUnitConsensus)
            disp('ack');
        end
        [~,~,~,xpoints1,ypoints12]=psth_wStd(filtspikes(useSpikes,0,'led',ledCond1{2}),bin,0,trialDuration,length(unitByUnitConsensus),unitByUnitConsensus);
        %%%%%%%%%%%%%%%%%%%%%%%%%%%
        prefPSTH_xpoints=xpoints1;
        prefPSTH_ypoints=zeros(length(useTheseAssigns),length(ypoints1));
        prefPSTH_ypoints(i,:)=ypoints1-ypoints12;
        useSpikes=filtspikes(spikes,0,'assigns',useTheseAssigns(i),'stimcond',type2Stimcond{1});
        unitByUnitConsensus=currTrialCon(ismember(currStimCon,type2Stimcond{1}) & ismember(currLEDCon,ledCond2{1}));
        if isempty(unitByUnitConsensus)
            disp('ack');
        end
        [~,~,~,xpoints1,ypoints1]=psth_wStd(filtspikes(useSpikes,0,'led',ledCond2{1}),bin,0,trialDuration,length(unitByUnitConsensus),unitByUnitConsensus);
        %%%%%%%%%%%%%
        useSpikes=filtspikes(spikes,0,'assigns',useTheseAssigns(i),'stimcond',type2Stimcond{2});
        unitByUnitConsensus=currTrialCon(ismember(currStimCon,type2Stimcond{2}) & ismember(currLEDCon,ledCond2{2}));
        if isempty(unitByUnitConsensus)
            disp('ack');
        end
        [~,~,~,xpoints1,ypoints12]=psth_wStd(filtspikes(useSpikes,0,'led',ledCond2{2}),bin,0,trialDuration,length(unitByUnitConsensus),unitByUnitConsensus);
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        nonprefPSTH_xpoints=xpoints1;
        nonprefPSTH_ypoints=zeros(length(useTheseAssigns),length(ypoints1));
        nonprefPSTH_ypoints(i,:)=ypoints1-ypoints12;
    else
        useSpikes=filtspikes(spikes,0,'assigns',useTheseAssigns(i),'stimcond',type1Stimcond{1});
        currTrialCon=unitByUnitTrials{i};
        currStimCon=unitByUnitStimcond{i};
