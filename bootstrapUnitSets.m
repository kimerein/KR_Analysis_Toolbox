function [saveStep1,saveStep2]=bootstrapUnitSets(allSpikes,assignsSet1,assignsSet2,unitByUnitTrials,unitByUnitStimcond,unitByUnitLED,useTheseStimcond,useOnlySigForStimUnits,PSTHbin,ledOnCond,stimResponseWindow,unitsFractionForBootstrap,trialDuration,saveStep1,saveStep2)

nBootstrapTrials=1000;
saveSteps=1;
ledOnset=3.3; % in seconds from the start of the trial
normalizePrecedingWindow=1;
precedingWindow=[3.3 3.307]; % s before thalamic silencing
zeroWindow=[3.38 3.42]; % For normalization, when traces should be at zero
...
% each trial
nUnitsPerTrial1=floor(unitsFractionForBootstrap*length(assignsSet1));
nUnitsPerTrial2=floor(unitsFractionForBootstrap*length(assignsSet2));
...
function [prefR,prefStimcond,nonprefR,nonprefStimcond,sigStim]=getSignificanceOfPreferredResponse(spikes,useTheseAssigns,useTheseStimcond,onsetResponseWindow,unitByUnitTrials,unitByUnitStimcond,nBest,nWorst)
