function plotStimsAndLEDCondsSeparately(baselineWindow,LFPbySweep,nStimParam1Vals,nStimParam2Vals,ledForSweeps,stimsForSweeps,LFP_Fs,totalTrialLength)
...
for i=1:nStimParam2Vals
    ...
    plot(0:totalTrialLength/(size(LFPbySweep,2)-1):totalTrialLength,mean(LFPbySweep(noLEDtrialsForConds{i},:),1));
    hold all;
end
axis([0 totalTrialLength  -0.3 0.1]);
hold off
% LED ON
subplot(2,1,2);
for i=1:nStimParam2Vals
    ...
    plot(0:totalTrialLength/(size(LFPbySweep,2)-1):totalTrialLength,mean(LFPbySweep(LEDtrialsForConds{i},:),1));
    hold all;
end
axis([0 totalTrialLength  -0.3 0.1]);
