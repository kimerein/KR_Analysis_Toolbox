function allPtoT=plotWaveformHistogram_andSortByTrialType(spikes,Fs,parameter)
...
if strcmp(parameter,'half-width')
    allPtoT=zeros(size(spikes.waveforms,1),1);
    thin_countLEDspiketimes=[];
    thin_countNoLEDspiketimes=[];
    reg_countLEDspiketimes=[];
    reg_countNoLEDspiketimes=[];
    thresh=3*10^-4;
    for i=1:size(spikes.waveforms,1)
        currEventCh=spikes.info.detect.event_channel(i);
...
%         end
        if spikes.led(i)==0
            if allPtoT(i)<thresh
                thin_countNoLEDspiketimes=[thin_countNoLEDspiketimes; spikes.spiketimes(i)];
            else
                reg_countNoLEDspiketimes=[reg_countNoLEDspiketimes; spikes.spiketimes(i)];
            end
        else
            if allPtoT(i)<thresh
                thin_countLEDspiketimes=[thin_countLEDspiketimes; spikes.spiketimes(i)];
            else
                reg_countLEDspiketimes=[reg_countLEDspiketimes; spikes.spiketimes(i)];
            end
        end
    end
    %allPtoT=allPtoT/Fs;
    figure();
    title('Thin Spikes');
    [n_noLED,xout_noLED]=hist(thin_countNoLEDspiketimes,50);
    [n_LED,xout_LED]=hist(thin_countLEDspiketimes,50);
    plot(xout_noLED,n_noLED,'Color','black');
    hold on;
    plot(xout_LED,n_LED,'Color','red');
    axis tight;
    figure();
    title('Reg Spikes');
    [n_noLED,xout_noLED]=hist(reg_countNoLEDspiketimes,50);
    [n_LED,xout_LED]=hist(reg_countLEDspiketimes,50);
    plot(xout_noLED,n_noLED,'Color','black');
    hold on;
    plot(xout_LED,n_LED,'Color','red');
    axis tight;
