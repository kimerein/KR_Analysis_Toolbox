...
for i=1:length(psth.psths)
    currp=psth.psths{i};
    l=psth.unitLED{1};
    bases=currp(ismember(l,ledVal),times>=baseline_window(1) & times<baseline_window(2));
    during_led=currp(ismember(l,ledVal),times>=ledStart & times<ledStart+direct_window);
    spont_no_led=currp(~ismember(l,ledVal),times>=ledStart & times<ledStart+direct_window);
    evoked_noLED=nanmean(currp(~ismember(l,ledVal),times>=stimWindow(1) & times<stimWindow(2)),2);
    evoked_LED=nanmean(currp(ismember(l,ledVal),times>=stimWindow(1) & times<stimWindow(2)),2);
